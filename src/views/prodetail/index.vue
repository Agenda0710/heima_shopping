<template>
  <div class="prodetail">
    ...
    <div class="info">
      <div class="title">
        <div class="price">
          <span class="now">￥{{ goodDetail.goods_price_min }}</span>
          <span class="oldprice">￥{{ goodDetail.goods_price_max }}</span>
        </div>
    ...
    <!-- 商品描述 -->
    <div class="desc" v-html="goodDetail.content">
    </div>
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      getProductDetail(this.getGoodsId).then((response) => {
    ...
    getComments() {
      getComments(this.getGoodsId, 3).then((response) => {
        const res = response.data;
